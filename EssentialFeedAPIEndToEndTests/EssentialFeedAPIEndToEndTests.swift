...
final class EssentialFeedAPIEndToEndTests: XCTestCase {
    ...
    func test_endToEndTestServerGETFeedResult_matchesFixedTestAccountData() {
        ...
        case let .success(imageFeed)?:
            ...
            imageFeed.enumerated().forEach { (index, item) in
                XCTAssertEqual(item, exptctedImage(at: index), "Unexpected item values at index \(index)")
                XCTAssertEqual(imageFeed[index], exptctedImage(at: index))
            }
    ...
    private func exptctedImage(at index: Int) -> FeedImage {
        FeedImage(
            id: id(at: index),
